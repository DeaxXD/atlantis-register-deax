...
fs.readdir('./komutlar/', (err, files) => {//
    ...
    log(`[deax] ${files.length} komut yüklenecek.`);//
    files.forEach(f => {//
        let props = require(`./komutlar/${f}`);//
        log(`[deax] Yüklenen komut: ${props.help.name}.`);//
...
client.on("guildMemberAdd", member => {  
 ...
 const hgmesaj = (`**${ayarlar.sunucuisim}'e__** hoşgeldin (<@${vader.id}>) - (\`${vader.id}\`)
   ...
   Hesabın (`  + moment(member.user.createdAt).format("DD MMMM YYYY dddd") + `) oluşturulduğu için`   + kontrol + `
    Kaydını tamamlamak için herhangi bir \`V.Confirmed\` teyit odasına girmen yeterlidir. 
     Kayıt olduktan sonra rol seçim katagorisinden rolleri almayı unutma. 
      Tagımızı alarak ailemizin bir parçası olabilirsin. \`${ayarlar.tag}\``)
kanal.send(hgmesaj)
